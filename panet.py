...
class SEModule(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc2(x)
        x = self.sigmoid(x)
        #torch.save(x, 'SEModel{}.pt'.format(x.shape[1])) 
        #print('SEModel {}'.)
...
class PaModule(nn.Module):
    ...
    def forward(self, x):
        ...
        #b,c,1,h+w
        x_cat_conv_split_h, x_cat_conv_split_w = x_cat_conv_relu.split([H, W], 3)#拆分卷积后的维度
        #b,c,1,h    b,c,1,w
        ...
        out = x  * s_w.expand_as(x) * se.expand_as(x) ##only x direction
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.pa_module(out) + residual
        out = self.relu(out)
        
        #print('outSEModel {}'.format(out.shape))

        return out


class PABottleneck(Bottleneck):
    """
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
        ...
        self.relu = nn.ReLU(inplace=True)
        self.pa_module = PaModule(planes * 4, reduction=reduction)
        self.downsample = downsample
        self.stride = stride


class PAResNetBottleneck(Bottleneck):
    """
    ...
    def __init__(self, inplanes, planes, groups, reduction, stride=1,
                 downsample=None):
        super(PAResNetBottleneck, self).__init__()
